...
export default async function getWalletHistory(address, coinID) {
  ...
  if (coinID === "rocket-pool") action = "/api?module=account&action=tokentx"
  ...
  try {
    ...
    let deposits = await Promise.all(history
